...
# ---------- Persistent Header ----------
st.markdown("""
    <div style='display:flex;justify-content:space-between;align-items:center;background:linear-gradient(to right, #e0f7fa, #e1bee7);padding:10px 20px;border-radius:5px;margin-bottom:20px;'>
        <h2 style='margin:0;text-align:center;width:100%;'>SmartMatch</h2>
        {}
    </div>
""".format(
    f"<button onclick='window.location.reload();'>Logout</button>" if st.session_state.get("authenticated") else ""
), unsafe_allow_html=True)
